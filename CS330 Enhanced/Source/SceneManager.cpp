...
float scene_counter = 1; // variable to control the scene thats choosen but is set to 1 by default
...
/*** for assistance.                                        ***/
/**************************************************************/
void SceneManager::LoadSceneTextures() { //imports textures and gives a tag to call when rendering to apply texture
	bool bReturn = false;
	...
	bReturn = CreateGLTexture(
		"textures/mesh.jpg",
		"mesh");
	BindGLTextures(); //bind the textures
}
...
void SceneManager::SetupSceneLights()
{
	...
	m_pShaderManager->setVec3Value("directionalLight.direction", -0.05f, 0.0f, -0.1f);
	...
	// point light 4
	m_pShaderManager->setVec3Value("pointLights[4].position", -3.2f, 6.0f, -2.0f);
	...
	m_pShaderManager->setVec3Value("pointLights[4].specular", 0.4f, 0.4f, 0.4f);
...
void SceneManager::PrepareScene()
{
	...
	SetupSceneLights();
	LoadSceneTextures(); //preloads the textures
...
void SceneManager::RenderScene()
{
	...
	if (scene_counter == 1) {
		...
		RenderPrimaryScene();// renders the original calls in a single function
	}
	else if (scene_counter == 2) {
		glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT | GL_STENCIL_BUFFER_BIT);
		RenderSecondScene(); //renders the second scene when called
	}
	/*RenderBackground();
	RenderMonitor();
	RenderTable();
	...
	RenderPencilCup();
	RenderKeyboard();
	RenderMouse();*/ //these were the original calls to render the scene
	//calls the objects to be rendered
	//add a new scene with the push of button
	//FAILSAFE to prevent the scenes from changing too quickly
}
float SceneManager::GetSceneCount() const {
	return scene_counter;
}
void SceneManager::RenderPrimaryScene()
{
	RenderBackground();
	RenderMonitor();
	RenderTable();
...
void SceneManager::RenderSecondScene()
{
	RenderBackground();
	RenderTV();
	Rendercouch();
...
void SceneManager::RenderTV() {
	...
	scaleXYZ = glm::vec3(20.0f, 12.0f, 0.5f);
	...
	positionXYZ = glm::vec3(0.0f, 6.0f, -9.0f);
	...
	SetShaderColor(0.0, 0, 0, 1);
	SetShaderMaterial("plastic");
	// draw the mesh with transformation values

	m_basicMeshes->DrawBoxMesh();
	scaleXYZ = glm::vec3(9.0f, 1.0f, 5.0f);
	...
	// set the XYZ position for the mesh
	positionXYZ = glm::vec3(0.0f, 6.0f, -8.5f);
	...
	// set the color values into the shader
	SetShaderTexture("dispaly");//sets texture
	SetTextureUVScale(1.0, 1.0);//sets texture to be repeated
...
void SceneManager::Rendercouch() {
	...
	/****************************************************************/
	scaleXYZ = glm::vec3(25.0f, 1.5f, 10.0f);
	...
	// set the XYZ position for the mesh
	positionXYZ = glm::vec3(0.0f, -8.0f, 6.0f);
...
========================================================================
*/
void SceneManager::RenderBackground() { //renders the floor and wall
	glm::vec3 scaleXYZ;
	float XrotationDegrees = 0.0f;
	float YrotationDegrees = 0.0f;
	float ZrotationDegrees = 0.0f;
	...
	SetShaderTexture("floor");//sets texture
	SetTextureUVScale(2.0, 3.0); //sets how many times the texture is repeated
	SetShaderMaterial("wood");
	// draw the mesh with transformation values
	m_basicMeshes->DrawPlaneMesh();
	/****************************************************************/
...
void SceneManager::RenderTable() {
	...
	// set the XYZ position for the mesh
	positionXYZ = glm::vec3(0.0f, -1.0f, -4.97f);
	...
	// set the color values into the shader
	SetShaderTexture("table"); //sets texture
	
	SetTextureUVScale(1.0, 2.0);//sets how many times texture repeats
	SetShaderMaterial("wood");
	...
	/****************************************************************/
	scaleXYZ = glm::vec3(12.0f, 0.2f, 5.0f);
	...
	// set the XYZ position for the mesh
	positionXYZ = glm::vec3(0.0f, -0.145f, -2.97f);
...
void SceneManager::RenderDesktop() {
	...
	/*** and drawing all the basic 3D shapes.						***/
	/******************************************************************/
	scaleXYZ = glm::vec3(4.0f, 7.0f, 7.0f);
	...
	// set the XYZ position for the mesh
	positionXYZ = glm::vec3(-9.0f, 3.8f, -3.8f);
	...
	m_basicMeshes->DrawBoxMesh();
	/****************************************************************/
	scaleXYZ = glm::vec3(0.5f, 0.55f, 0.5f);
	...
	// set the XYZ position for the mesh
	positionXYZ = glm::vec3(-10.0f, -0.23f, -1.5f);
	...
	// set the XYZ position for the mesh
	positionXYZ = glm::vec3(-8.0f, -0.23f, -1.5f);
	...
	// set the XYZ position for the mesh
	positionXYZ = glm::vec3(-8.0f, -0.23f, -6.5f);
	...
	// set the XYZ position for the mesh
	positionXYZ = glm::vec3(-10.0f, -0.23f, -6.5f);
...
void SceneManager::RenderMonitor() {
	...
	/*** and drawing all the basic 3D shapes.						***/
	/******************************************************************/
	scaleXYZ = glm::vec3(12.0f, 4.0f, 0.5f); //resize the cylinderMesh so that the shapes above dont look out of place
	...
	// set the XYZ position for the mesh
	positionXYZ = glm::vec3(0.0f, 3.5f, -6.5f); //position of the cyclinder with in the scene
	...
	// set the color values into the shader
	SetShaderColor(0.0, 0, 0.0, 1); //sets color of the cylinder
	SetShaderMaterial("plastic");
	// draw the mesh with transformation values

	m_basicMeshes->DrawBoxMesh();
	/****************************************************************/
	scaleXYZ = glm::vec3(0.5f, 5.0f, 1.0f); //resize the cylinderMesh so that the shapes above dont look out of place
	...
	// set the XYZ position for the mesh
	positionXYZ = glm::vec3(0.0f, 2.5f, -7.5f); //position of the cyclinder with in the scene
	...
	m_basicMeshes->DrawBoxMesh();
	/****************************************************************/
	scaleXYZ = glm::vec3(4.0f, 0.5f, 0.5f); //resize the cylinderMesh so that the shapes above dont look out of place
	...
	// set the XYZ position for the mesh
	positionXYZ = glm::vec3(-1.5f, 0.75f, -7.0f); //position of the cyclinder with in the scene
	...
	m_basicMeshes->DrawBoxMesh();
	/****************************************************************/
	scaleXYZ = glm::vec3(4.0f, 0.5f, 0.5f);
	...
	m_basicMeshes->DrawBoxMesh();
	/****************************************************************/
	scaleXYZ = glm::vec3(2.0f, 2.0f, 0.5f);
	...
	m_basicMeshes->DrawBoxMesh();
	scaleXYZ = glm::vec3(5.8f, 1.0f, 1.7f);
	...
	// set the XYZ position for the mesh
	positionXYZ = glm::vec3(0.0f, 3.5f, -6.245f);
...
void SceneManager::RenderPencilCup() {
	...
	// set the XYZ position for the mesh
	positionXYZ = glm::vec3(8.0f, -0.235f, -6.245f);
	...
	// set the color values into the shader
	SetShaderTexture("mesh");//sets texture
	SetTextureUVScale(1.5, 1.02);//sets texture to be repeated
...
void SceneManager::RenderKeyboard()
{
	...
	// set the XYZ position for the mesh
	positionXYZ = glm::vec3(-1.0f, 0.2f, -3.245f);
	...
	m_basicMeshes->DrawBoxMesh();
	/**************************************************************/
	scaleXYZ = glm::vec3(0.3f, 0.4f, 0.2f);
	...
	// set the XYZ position for the mesh
	positionXYZ = glm::vec3(-3.7f, 0.2f, -3.945f);
	...
	// set the XYZ position for the mesh
	positionXYZ = glm::vec3(1.65f, 0.2f, -3.945f);
...
void SceneManager::RenderMouse() {
	...
	scaleXYZ = glm::vec3(0.75f, 0.75f, 1.3f);
	...
	// set the XYZ position for the mesh
	positionXYZ = glm::vec3(4.0f, -0.05f, -3.245f);
	...
	// set the XYZ position for the mesh
	positionXYZ = glm::vec3(4.15f, 0.45f, -3.7f);
